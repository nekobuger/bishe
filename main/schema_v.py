...
def schemaName_spider(request, tableName):
    if request.method in ["POST", "GET"]:
        ...
        # Linux
        cmd = "cd /yykj/python/9999/spider${spiderSchemaName} && scrapy crawl "+tableName+"Spider -a databaseName=django"
        # Windows
        # cmd = "cd C:\\test1\\spider && scrapy crawl " + tableName + "Spider -a databaseName=django"
        os.system(cmd)
...
def schemaName_mysqldump(request):
    if request.method in ["POST", "GET"]:
        ...
        cmd = "/usr/bin/mysqldump -h127.0.0.1 -uroot -P3306 -p123456 django > " + filePath
        os.system(cmd)
