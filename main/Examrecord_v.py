...
def examrecord_page(request):
    ...
    if request.method in ["POST", "GET"]:
        ...
        if  __isAdmin__ == "是" and 'examrecord' != 'forum':
            if req_dict.get("userid") and 'examrecord' != 'chat':
                del req_dict["userid"]
        else:
...
def examrecord_save(request):
    ...
    if request.method in ["POST", "GET"]:
        ...
        paperInfo = exampaper.objects.filter(id=req_dict.get('paperid'))
        for data in paperInfo.values('jiaoshigonghao'):
            attrName = data['jiaoshigonghao']
            req_dict['jiaoshigonghao'] = attrName
...
def examrecord_o_value(request, xColumnName, yColumnName):
    if request.method in ["POST", "GET"]:
        ...
        sql = "SELECT {0}, ROUND(sum({1}),2) AS total FROM examrecord {2} GROUP BY {0} LIMIT 10".format(xColumnName, yColumnName, where)
        L = []
        cursor = connection.cursor()
        cursor.execute(sql)
        desc = cursor.description
