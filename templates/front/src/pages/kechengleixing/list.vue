...
<div>
    ...
    <el-form :inline="true" :model="formSearch" class="list-form-pv" :style='{"padding":"10px 0 10px","alignItems":"center","flexWrap":"wrap","background":"none","display":"flex","width":"100%","height":"auto","order":"1"}'>
      <el-form-item :style='{"margin":"0 10px 0px 0"}'>
	    <div class="lable" v-if="true" :style='{"width":"auto","padding":"0 10px 0 0","lineHeight":"42px","display":"inline-block"}'>课程类型：</div>
	  ...
	  <el-button v-if="btnAuth('kechengleixing','新增')" :style='{"cursor":"pointer","border":"0","padding":"0px 15px","margin":"0 10px 0 0","outline":"none","color":"#fff","borderRadius":"4px","background":"#000","width":"auto","fontSize":"14px","lineHeight":"42px","height":"42px"}' type="primary" @click="add('/index/kechengleixingAdd')"><i v-if="true" :style='{"color":"#fff","margin":"0 10px 0 0","fontSize":"14px"}' class="el-icon-circle-plus-outline"></i>添加</el-button>
    </el-form>
		...
		<div class="list2 index-pv1" :style='{"padding":"0 0","flexWrap":"wrap","background":"#fff","display":"flex","width":"100%","justifyContent":"space-between","height":"auto"}'>
			<div :style='{"padding":"25px","boxShadow":"0px 4px 10px 0px rgba(0,0,0,0.3)","margin":"10px 0","background":"#fff","display":"flex","width":"calc(50% - 10px)","fontSize":"0","position":"relative","height":"auto"}' v-for="(item, index) in dataList" :key="index" @click.stop="toDetail(item)" class="list-item animation-box">
				<div :style='{"border":"0","width":"150px","padding":"0","height":"110px"}'>
					<img @click.stop="imgPreView(item.image)" :style='{"border":"1px solid #eee","width":"100%","objectFit":"contain","background":"#fff","display":"block","height":"100%"}' v-if="item.image && item.image.substr(0,4)=='http'" :src="item.image.split(',')[0]" class="image" />
					<img @click.stop="imgPreView(baseUrl + (item.image?item.image.split(',')[0]:''))" :style='{"border":"1px solid #eee","width":"100%","objectFit":"contain","background":"#fff","display":"block","height":"100%"}' v-else :src="baseUrl + (item.image?item.image.split(',')[0]:'')" class="image" />
				</div>
