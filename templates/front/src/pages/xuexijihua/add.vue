...
<div :style='{"width":"80%","padding":"20px 0","margin":"0 auto","position":"relative","background":"#FCFAFF"}'>
    <el-form
      ...
      label-width="100px"
    >
          <el-form-item :style='{"padding":"10px","margin":"0 0 10px","background":"none"}' label="计划标题" prop="jihuabiaoti">
            <el-input v-model="ruleForm.jihuabiaoti" 
                placeholder="计划标题" clearable :disabled=" false  ||ro.jihuabiaoti"></el-input>
          </el-form-item>
          ...
          <el-form-item :style='{"padding":"10px","margin":"0 0 10px","background":"none"}' label="计划时间" prop="jihuashijian">
            <el-input v-model="ruleForm.jihuashijian" 
                placeholder="计划时间" clearable :disabled=" false  ||ro.jihuashijian"></el-input>
          </el-form-item>
          ...
          <el-form-item :style='{"padding":"10px","margin":"0 0 10px","background":"none"}' label="计划目标" prop="jihuamubiao">
            <el-input
              type="textarea"
              :rows="8"
              placeholder="计划目标"
...
<script>
  export default {
    ...
    // 多级联动参数
      // 多级联动参数
      info() {
        this.$http.get(`xuexijihua/detail/${this.$route.query.id}`, {emulateJSON: true}).then(res => {
          if (res.data.code == 0) {
					...
					if(crossrefid && crossuserid) {
						...
						this.$http.get('xuexijihua/list', {
							...
							if(res.data.data.total>=crossoptnum) {
								...
								this.$http.post(`xuexijihua/${this.ruleForm.id?'update':this.centerType?'save':'add'}`, this.ruleForm).then(res => {
									if (res.data.code == 0) {
										this.$message({
											message: '操作成功',
											type: 'success',
